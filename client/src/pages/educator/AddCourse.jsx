  ...
  return (
    <div className='h-screen overflow-scroll flex flex-col items-start justify-between md:p-8 md:pb-0'>
        <form onSubmit={handleSubmit}>
          ...
          <div className='flex flex-col gap-1'>
              <p>Discount %</p>
              <input onChange={e=>setDiscount(e.target.value)}  type="number" value={discount} placeholder='0' max={100} min={0}
               className='outline-none md:py-2.5 py-2 w-28 px-3 rounded border border-gray-500' required
              />
          </div>
          <div className='pt-3'>
            {
             chapters.map((chapter, chapterIndex)=>(
              <div key={chapterIndex} className='bg-white border border-gray-500 rounded-lg mb-4 '>
                <div className='flex justify-between items-center p-4 border-b border-gray-500/20'>
                    <div className='flex items-center gap-1'>
                      <img src={assets.dropdown_icon}  onClick={()=>handleChapter('toggle', chapter.chapterId)} className='cursor-pointer' alt="" width={14}/>
            ...
            {
              showPopup && (
                <div className='fixed inset-0 flex items-center justify-center bg-gray-800/20 bg-opacity-50'>
                    <div className='bg-white text-gray-700 p-4 rounded relative w-full max-w-80'>
                       ...
                       <button onClick={addLecture} type='button' className='w-full cursor-pointer bg-blue-400 text-white px-4 py-2 rounded'>Add</button>
                       <img src={assets.cross_icon} onClick={()=>setPopup(false)} className=' absolute top-4 right-4 w-4 cursor-pointer' alt="" />
                    </div>
                </div>
              )
            }
          </div>
          <button type='submit' className=' bg-blue-600 cursor-pointer text-white px-8 mt-3 py-2 rounded'>Add</button>
        </form>       
    </div>
  )
}
